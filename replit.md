...
**Enrichment Pipeline v2 (Feb 2026):**
    ...
    - **Suspicious name detection (v3):** _is_suspicious_name checks ALL name words for consonant-only words, 30+ unusual bigrams, unusual trigrams, vowel ratio <0.25, repeated characters. _is_valid_contact_name consonant cluster threshold tightened to 5. Catches gibberish like "Jfm Ixro", "Kpn Awwp". When suspicious name found with valid team alternatives, assigns best team member and logs replacement in refinement_notes.
    ...
    - **Email classification (v3):** email_type checks primary email field, contact_email field, AND generic_email field. Also considers team_email_guesses and principal_email_guess for 'guessed' classification. Personal classification requires name-part match in email local part or non-guessed status.
    ...
    - **Company name contact extraction:** When no valid contact_name exists, extracts practitioner names from company names (e.g., "Dr Melanie Orchard - Clinical Psychologist" → Melanie Orchard, "Trish Morgan Counselling" → Trish Morgan). Uses ~170 common UK first names whitelist, strips titles/business suffixes, handles hyphenated surnames. Logs contact_from_company_name in refinement_notes, sets contact_source='company_name'.
    ...
    - **Enhanced email guess validation:** 8 patterns (firstname.lastname@, f.lastname@, firstname@, lastname@, firstname_lastname@, firstnamelastname@, firstnamel@, fl@), regex-validated, domain checked for TLD, lowercase normalized, deduplicated.
    - **Separate personal/team email guesses:** Primary contact guesses → personal_email_guesses, other team members → team_email_guesses with cross-deduplication.
    - **Excluded lead tagging:** Leads with no website and no Facebook get 'excluded:no_web_no_fb' in refinement_notes, confidence_score=1, mailshot_category=do_not_email.
