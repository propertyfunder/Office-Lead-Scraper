...
## User Preferences
The user wants to interact with the system via a Command Line Interface (CLI) for scraping and a Flask web dashboard for lead visualization and management. The dashboard includes pipeline control buttons to run each search independently (Office Lead Search and Unit 8 Wellness Search) without needing the CLI. The user prefers to specify search parameters such as target towns, sectors, and run modes (e.g., wellness mode) via CLI arguments. Output should be in CSV format. The user requires detailed logging for debugging purposes and the ability to perform dry runs without saving data. The user values detailed explanations of AI scores and reasons.

## System Architecture
The system employs a modular Python architecture for scraping, enrichment, AI scoring, and a web-based dashboard.

**UI/UX Decisions:**
- **Web Dashboard:** A Flask application provides a web interface for lead management with two tabs for lead categorization (Unit 8 Occupiers and Office Occupiers). It offers filtering by AI score, company name, sector, or location, displays aggregated statistics, and allows data download as CSV, including review flags and data quality scores. Pipeline output is tee'd to both the log file (visible in the UI log viewer at 500px tall, 100-line tail) and to Flask's stdout (visible in the Replit console with `[OFFICE]`/`[UNIT8]` prefixes).
...
- **Scraping:** Two primary pipelines: (1) Google Places API for wellness/Unit 8 leads, (2) Companies House Advanced Search API for office occupier leads via SIC codes. CH API calls are rate-limited to a minimum 0.6s gap between requests to stay within the ~600 req/5min quota.
- **Office Pipeline:** `python main.py --mode office` uses `CHOfficeDiscoveryScraper` (SIC code + GU postcode discovery) as primary, with `PlacesCrossReference` for website/phone lookup. Output: `office_leads.csv`. Config in `config.py`. Uses fast enrichment path: when CH provides a valid director name, only the homepage is scraped (1 request vs 9) and personal email guesses are generated from director name + domain. Falls through to full enrichment only when no director name exists or homepage yields no email. Now stores `date_of_creation` from Companies House for size classification.
- **Post-CH Email Enrichment Pipeline:** `run_office_enrichment.py` runs after the CH sweep completes. Phase 2: discovers websites via Google Places for records missing them. Phase 3: classifies leads by `size_signal` (larger/established_small/small_new) using team page detection and company age, then applies tiered email strategy — larger companies get team page scraping, established small get homepage-only with generic email capture, small/new get director-name email guessing only. Phase 4: runs geo classification on newly enriched records. Dashboard button: "Run Email Enrichment".
